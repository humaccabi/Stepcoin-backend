...
public class JSON
{
                ...
                for (;context.getIndex() < len;) {
                    ...
                    if (StringTools.isHexDigit(d)) { // A/B/C/D/F (0..9 and 'E' handled above)
                        if (!hexCh) {
                            ...
                            throw new JSONParsingException("Invalid numeric value (invalid hex value)", context);
                        }
                        hexDig++; // hex digits after 'x'
                        num.append(d);
                        context.incrementIndex();
                    ...
                    if ((d == 'x') || (d == 'X')) {
                        if (hexCh) {
                            // -- more than one 'x'
                            throw new JSONParsingException("Invalid numeric value (more than one 'x')", context);
                        } else
    ...
    **/
    ...
    {
        if (file != null) {
            String json = StringTools.toStringValue(FileTools.readFile(file));
            /*mdf*/Print.logInfo("JSON file:\n"+json);
            this.object = JSON.parse_Object(json);
        }
    }
