        ...
        implements SessionInfo
    {
            ...
            if ((data != null) && (data.length > 0)) {
                synchronized (this.runLock) {
                    ...
                    if (this.client.isTCP()) {
                        if (LogEnable) { Print.logInfo("("+this._getName()+") UDP] Ignoring TCP write: 0x%s", StringTools.toHexString(data)); }
                    } else
                    ...
                    if (this.client.isInputStream()) {
                        if (LogEnable) { Print.logInfo("("+this._getName()+") UDP] Ignoring InputStream write: 0x%s", StringTools.toHexString(data)); }
                    } else {
                        // ?
                    }
                }
        ...
        **/
        ...
        public void handleClientSession(ClientSocket clientSock) {
            ...
            ClientPacketHandler clientHandler = ServerSocketThread.this.getClientPacketHandler(); // new instance for most DCS modules
            ...
            if (clientHandler != null) {
                try {
                    ...
                    if ((finalPacket != null) && (finalPacket.length > 0)) {
                        if (clientSock.isTCP()) {
                            // -- TCP: Send response over socket connection
                            if (LogEnable) { Print.logInfo("("+this._getName()+") TCP] Final Packet: 0x"+StringTools.toHexString(finalPacket)); }
